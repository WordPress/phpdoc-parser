...
class Importer {
	...
	public function import( array $data, $skip_sleep = false, $import_internal_functions = false ) {
		...
		$this->log( 'Starting import. This will take some time…' );
		...
		if ( ! post_type_exists( $this->post_type_class ) || ! post_type_exists( $this->post_type_function ) || ! post_type_exists( $this->post_type_hook ) ) {
			$this->error( sprintf( 'Missing post type; check that "%1$s", "%2$s", and "%3$s" are registered.', $this->post_type_class, $this->post_type_function, $this->post_type_hook ) );
			exit;
		}
	...
	protected function import_version( $data ) {
		...
		if ( isset( $wp_version ) && $wp_version ) {
			...
			$this->log( "\t" . sprintf( 'Updated option wp_parser_imported_wp_version to "%1$s"', $wp_version ) );
		}
	}
	...
	public function import_item( array $data, $parent_post_id = 0, $import_internal = false, array $arg_overrides = array() ) {
		...
		if ( ! $import_internal && wp_list_filter( $data['doc']['tags'], array( 'name' => 'internal' ) ) ) {
			...
			switch ( $post_data['post_type'] ) {
				case $this->post_type_class:
					$this->log( "\t" . sprintf( 'Skipped importing @internal class "%1$s"', $data['name'] ) );
					break;

				case $this->post_type_method:
					$this->log( "\t\t" . sprintf( 'Skipped importing @internal method "%1$s"', $data['name'] ) );
					break;

				case $this->post_type_hook:
					$indent = ( $parent_post_id ) ? "\t\t" : "\t";
					$this->log( $indent . sprintf( 'Skipped importing @internal hook "%1$s"', $data['name'] ) );
					break;

				default:
					$this->log( "\t" . sprintf( 'Skipped importing @internal function "%1$s"', $data['name'] ) );
			}
		...
		foreach ( $packages as $pack_name => $pack_value ) {
			...
			if ( is_wp_error( $package_term ) ) {
				if ( is_int( $main_package_id ) ) {
					$this->warn( "\tCannot create @subpackage term: " . $package_term->get_error_message() );
				} else {
					$this->warn( "\tCannot create @package term: " . $package_term->get_error_message() );
				}
			}
		}
		$added_term_relationship = did_action( 'added_term_relationship' );
		...
		if ( $is_new_post ) {
			switch ( $post_data['post_type'] ) {
				case $this->post_type_class:
					$this->log( "\t" . sprintf( 'Imported class "%1$s"', $data['name'] ) );
					break;

				case $this->post_type_hook:
					$indent = ( $parent_post_id ) ? "\t\t" : "\t";
					$this->log( $indent . sprintf( 'Imported hook "%1$s"', $data['name'] ) );
					break;

				case $this->post_type_method:
					$this->log( "\t\t" . sprintf( 'Imported method "%1$s"', $data['name'] ) );
					break;

				default:
					$this->log( "\t" . sprintf( 'Imported function "%1$s"', $data['name'] ) );
			}
		} else {
			switch ( $post_data['post_type'] ) {
				case $this->post_type_class:
					$this->log( "\t" . sprintf( 'Updated class "%1$s"', $data['name'] ) );
					break;

				case $this->post_type_hook:
					$indent = ( $parent_post_id ) ? "\t\t" : "\t";
					$this->log( $indent . sprintf( 'Updated hook "%1$s"', $data['name'] ) );
					break;

				case $this->post_type_method:
					$this->log( "\t\t" . sprintf( 'Updated method "%1$s"', $data['name'] ) );
					break;

				default:
					$this->log( "\t" . sprintf( 'Updated function "%1$s"', $data['name'] ) );
			}
		}
